...
import CollectionFilterTime from './CollectionFilterTime';
import { SeamoreContext } from '../../../SeamoreContext/SeamoreContext';
export default function CollectionListBody() {
  const { nftInfo, getDataByCollection, setCollectionSet } = React.useContext(SeamoreContext);
  useEffect(() => {
    (async () => {
      await getDataByCollection();
    })()
  }, [])
        ...
        <table className="w-full text-sm text-left text-gray-500">
          ...
          <tbody className="nft-table-body">

            {
              ...
              nftInfo.map((val,index) => (
                <tr key={index} className=" border-b border-gray-600">

                  <th scope="row" className="px-6 py-4 font-medium text-gray-900 ">
                    <div className="flex">
                      <img src={val.image_url} alt="img" className=" hover:border-4 border-blue-500 w-[45px] h-[45px] rounded-full" />
                      <div className="flex flex-col ml-5">
                        <p className="cursor-pointer hover:border-b-2 border-blue-500 mb-2">
                          <Link
                          onClick={() => {setCollectionSet(val)}}
                          to={`/collection/${val.name}`}
                          >
                            {val.name}
                          </Link>
                        </p>
