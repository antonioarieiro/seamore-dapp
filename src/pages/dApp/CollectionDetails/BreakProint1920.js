        ...
        <div className="mt-[24px] flex w-full h-[60px] bg-[#262338]">
          <div className="flex h-[56px] items-center justify-between w-full">
            ...
            <div className="mr-[16px]">
              Time:
            {floor_price.map(val => (<span key={val} onClick={() => { setFloor(val) }} className={floor === val ? 'filter-trade-options ml-12 px-[20px] py-[12px] bg-[#4E4B66] cursor-pointer' : 'cursor-pointer filter-trade-options ml-12 p-[6px] text-[#6E7191]'}>{val}</span>))}
            </div>
          </div>
        </div>
